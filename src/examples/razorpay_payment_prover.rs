...
//const SERVER_DOMAIN: &str = "proof.codes";
const SERVER_DOMAIN: &str = "api.razorpay.com";

// Setting of the notary server — make sure these are the same with those in ../../../notary-server
const NOTARY_HOST: &str = "notary.proof.codes";
...
async fn main() {
    ...
    let api_key = env::var("USER_API_KEY").unwrap();
    let api_secret = env::var("USER_API_SECRET").unwrap();
    ...
    let auth_base64 = general_purpose::STANDARD.encode(api_key + ":" + &api_secret);
    let request = Request::builder()
        .uri(format!(
            "https://{SERVER_DOMAIN}/v1/payments/{payment_id}"
//            "https://{SERVER_DOMAIN}/webhook/v1/payments/{payment_id}"
        ))
        ...
        .header("Authorization", "Basic ".to_owned() + &auth_base64)
    ...
    let (public_ranges, private_ranges) =
        find_ranges(prover.sent_transcript().data(), &[payment_id.as_bytes()]);
    ...
    let mut file = tokio::fs::File::create("razporpay__notarized_session.json")
        .await
        .unwrap();
    file.write_all(
    ...
    let mut file = tokio::fs::File::create("razorpay_payment_proof.json")
        .await
        .unwrap();
...
async fn setup_notary_connection() -> (tokio_rustls::client::TlsStream<TcpStream>, String) {
    ...
    let pem_file =
        str::from_utf8(include_bytes!("../../test-certs/rootCA.crt")).unwrap();
